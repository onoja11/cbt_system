...
import { ShieldAlert, Users, CheckCircle, Search, RotateCcw, ArrowLeft, Loader2, RefreshCw, Clock } from 'lucide-react';
...
export default function LiveMonitor({ assessmentId, onNavigateBack }) {
  ...
  const [roomSecondsLeft, setRoomSecondsLeft] = useState(null);
  ...
  const fetchRoomTelemetry = async () => {
    try {
      ...
      if (response.ok && data.status === 'SUCCESS') {
        ...
        // 🎯 FIXED: ROBUST MULTI-KEY TIMESTAMPER PARSER WITH PASSIVE FALLBACKS
        const rawScheduledEnd = 
          data.assessment_meta?.scheduled_end || 
          data.assessment?.scheduled_end || 
          data.session?.scheduled_end || 
          data.exam?.scheduled_end || 
          data.scheduled_end;

        if (rawScheduledEnd && !examEndTimeRef.current) {
          const targetTimeString = String(rawScheduledEnd).replace(' ', 'T');
          examEndTimeRef.current = new Date(targetTimeString).getTime();
        } else if (data.students && data.students.length > 0 && !examEndTimeRef.current) {
          // 🎯 PROCTION FALLBACK: If DB timestamp keys are empty, calculate end-time from active students
          const maxStudentSeconds = Math.max(...data.students.map(s => parseInt(s.seconds_remaining, 10) || 0));
          if (maxStudentSeconds > 0) {
            examEndTimeRef.current = Date.now() + (maxStudentSeconds * 1000);
          }
        }
      }
    } catch (error) {
  ...
  // Linear interval countdown engine
  useEffect(() => {
    const tickerInterval = setInterval(() => {
      ...
      if (remainingSeconds <= 0) {
        setRoomSecondsLeft(0);
      } else {
        setRoomSecondsLeft(remainingSeconds);
      }
    }, 1000);
  ...
  const executeNuclearResetRestart = async (studentProfileId) => {
    try {
      const response = await apiRequest(`api/v1/teacher/monitor/workstation/restart-exam`, {
        method: 'POST',
        body: JSON.stringify({
          assessment_id: parseInt(assessmentId, 10),
          student_profile_id: parseInt(studentProfileId, 10)
      ...
      if (response.ok) {
        triggerBannerAlert("Exam session flushed completely. Student workstation ready for a clean reload!", "success");
        await fetchRoomTelemetry();
      }
    } catch (err) {
      console.error(err);
    ...
    try {
      ...
      if (response.ok) {
        if (examEndTimeRef.current) {
          examEndTimeRef.current += (parseInt(minutes, 10) * 60 * 1000);
        }
        triggerBannerAlert(studentProfileId ? `Added +${minutes} extra minutes.` : `Added +${minutes} extra minutes to ALL active students!`, "success");
        await fetchRoomTelemetry();
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-[#FAF9FA] text-[#2A1A63] font-sans flex flex-col justify-between select-none relative w-full overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full">
          <div className="flex items-center gap-2">
            ...
            <div className="text-left">
              <h2 className="text-xs font-black uppercase tracking-wider text-slate-950">Live Exam Hall Monitor</h2>
      ...
      {notification.isVisible && (
        <div className="fixed top-20 right-6 z-[1000] p-4 border border-emerald-200 text-emerald-800 bg-emerald-50 rounded-xl flex items-center gap-3 max-w-sm shadow-xl font-mono text-xs uppercase">
          <CheckCircle className="w-4 h-4 shrink-0 text-emerald-600" />
          <div className="font-sans font-bold">{notification.message}</div>
        </div>
      )}

      <div className="w-full max-w-7xl mx-auto px-4 md:px-6 mt-6 grid grid-cols-1 sm:grid-cols-4 gap-4 shrink-0 text-left">
        <div className="p-4 bg-white border border-[#9A87A9]/20 rounded-xl flex items-center gap-3 shadow-3xs bg-indigo-50/10 border-indigo-100">
          <Clock className="w-5 h-5 text-[#2A1A63]" />
          <div>
            <span className="text-[9px] font-black text-[#9A87A9] uppercase font-mono block">Room Time Left</span>
            <p className={`text-xs font-black font-mono ${roomSecondsLeft !== null && roomSecondsLeft <= 300 ? 'text-[#C62927] animate-pulse' : 'text-slate-950'}`}>
              {roomSecondsLeft !== null ? formatSecondsToMinutes(roomSecondsLeft) : 'Synchronizing...'}
            </p>
          </div>
        </div>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 md:p-6 flex flex-col justify-start">
        <div className="bg-white border border-[#9A87A9]/30 rounded-xl p-4 md:p-5 flex flex-col h-[460px] overflow-hidden shadow-3xs w-full">
          <div className="relative mb-4 max-w-xs shrink-0 w-full text-left">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#9A87A9]" />
            <input type="text" value={searchFilter} onChange={(e) => setSearchFilter(e.target.value)} placeholder="Search student name..." className="w-full pl-9 pr-3 py-2 bg-[#FAF9FA] border border-[#9A87A9]/40 text-xs font-bold text-slate-950 rounded-lg uppercase" />
          </div>
              ...
              <table className="w-full border-collapse text-left text-xs font-medium text-slate-700">
                ...
                <tbody className="divide-y divide-slate-100 bg-white font-bold text-slate-700">
                  {filteredStudents.map((st, idx) => {
                    ...
                    const violationsCount = parseInt(st.violations, 10) || 0;
                    const isSubmitted = st.status.includes('Submitted') || st.is_submitted == true;
                    const isMalpracticeLockout = st.status.includes('Malpractice') || violationsCount >= 3;
                    ...
                    return (
                      <tr key={studentProfileId || idx} className="hover:bg-[#FAF9FA]/30 transition-all">
                        ...
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded font-mono text-[10px] font-black ${isMalpracticeLockout ? 'bg-[#C62927] text-white animate-pulse' : violationsCount > 0 ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-[#FAF9FA] text-[#9A87A9]'}`}>
                          ...
                          <div className="flex gap-1.5 justify-end items-center">
                            ...
                            {violationsCount > 0 && !isSubmitted && (
                              <div className="flex gap-1">
                                <button onClick={() => setModalConfig({ isOpen: true, type: 'clear_override', studentId: studentProfileId, studentName: nameString })} className="px-2 py-1 bg-amber-500 hover:opacity-95 text-white text-[10px] font-bold uppercase rounded-md flex items-center gap-1 cursor-pointer shadow-sm"><RotateCcw className="w-3 h-3 text-white" /> Clear Strikes</button>
                                <button onClick={() => setModalConfig({ isOpen: true, type: 'nuclear_restart', studentId: studentProfileId, studentName: nameString })} className="px-2 py-1 bg-[#C62927] hover:opacity-95 text-white text-[10px] font-bold uppercase rounded-md flex items-center gap-1 cursor-pointer shadow-sm"><RefreshCw className="w-3 h-3 text-white" /> Restart Exam</button>
                              </div>
                            )}
                            {isSubmitted && <span className="text-[10px] font-mono font-black border px-2 py-0.5 rounded-md bg-[#FAF9FA] border-[#9A87A9]/30 text-[#9A87A9]">✓ SUBMITTED</span>}
                          </div>
                        </td>
                      </tr>
                    );
      ...
      <ConfirmationModal 
        ...
        title={modalConfig.type === 'nuclear_restart' ? "☢️ CRITICAL HARD RESTART OVERRIDE" : modalConfig.type?.startsWith('mass_add') ? "Global Class Extra Time Allocation" : modalConfig.type?.startsWith('add_time') ? "Individual Extra Time Allocation" : "Reset Station Constraints"}
        message={
          modalConfig.type === 'nuclear_restart' ? `CRITICAL SYSTEM INTERCEPT: Wiping and hard-restarting the active exam sheet layout row for candidate ${modalConfig.studentName}? This completely flushes their synced answers database records and lets them re-start from page 1 fresh!` :
          modalConfig.type?.startsWith('mass_add') ? `Confirm adding +${modalConfig.addedMinutes} extra minutes onto EVERY single student currently active?` : 
          modalConfig.type?.startsWith('add_time') ? `Confirm pushing +${modalConfig.addedMinutes} extra minutes directly to student ${modalConfig.studentName}?` : 
          `Are you sure you want to clear the warning strikes for student ${modalConfig.studentName}? This will instantly unlock their browser screen layout and drop strikes back to 0.`
        }
        confirmLabel={modalConfig.type === 'nuclear_restart' ? "Wipe Progress & Restart Fresh" : modalConfig.type?.startsWith('mass_add') ? "Add Time to All" : modalConfig.type?.startsWith('add_time') ? "Add Extra Time" : "Unlock Student Computer"}
        cancelLabel="Cancel"
        onConfirm={
          modalConfig.type === 'nuclear_restart' ? () => executeNuclearResetRestart(modalConfig.studentId) :
        ...
        onCancel={closeModal}
        summaryData={{ "Target Student": modalConfig.studentName, "Action Context": "MONITOR_ADJUSTMENT_OVERRIDE" }}
      />
      <footer className="w-full border-t border-[#9A87A9]/20 bg-white py-2.5 text-center text-[9px] font-black text-[#9A87A9] tracking-wider font-mono uppercase shrink-0">Start-Rite Schools Corporate Monitor Matrix Systems Layer © 2026</footer>
    </div>
  );
}